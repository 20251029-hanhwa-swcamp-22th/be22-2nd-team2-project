...
|:---:|:---:|:---:|:---:|:---:|:---|
| **FR-001** | 회원 | 가입 | 입력 | 회원가입 | 이메일(ID), 비밀번호, 필명(Nickname)을 입력하여 가입한다. 이메일/필명 중복 불가. |
| **FR-002** | 회원 | 가입 | 검증 | 유효성 검사 | 비밀번호 8자 이상, 특수문자 포함. 필명은 비속어 필터링 적용. |
| **FR-003** | 회원 | 인증 | 로그인 | 로그인처리 | 등록된 이메일/비밀번호로 인증 성공 시 JWT/세션 토큰 발급. |
| **FR-004** | 회원 | 인증 | 로그아웃 | 세션 종료 | 로그아웃 시 클라이언트 토큰 폐기 및 서버 세션 만료. |
| **FR-005** | 회원 | 마이페이지 | 조회 | 내 정보 | 내가 쓴 소설, 내가 쓴 문장, 좋아요 누른 작품 목록 조회. |
| **FR-006** | 회원 | 마이페이지 | 수정 | 프로필 수정 | 비밀번호 변경 및 필명 수정 기능 (변경 시 중복 체크 재수행). |
| **FR-007** | 회원 | 탈퇴 | 탈퇴 | Soft Delete | 회원 탈퇴 시 DB에서 완전히 삭제하지 않고 `status='DELETED'` 처리, 닉네임 '알수없음' 변경. |
| **FR-101** | 소설 | 생성 | 설정 | 소설 방 만들기 | 제목, 장르, 표지(옵션), 최대 문장 수(10~100)를 설정하여 생성. |
| **FR-102** | 소설 | 생성 | 시작 | 첫 문장 | 소설 생성 시 첫 문장(오프닝)은 방장이 작성해야 한다. |
| **FR-103** | 소설 | 조회 | 목록 | 소설 목록 | 최신순, 인기순(좋아요), 장르별 필터링 목록 조회. |
| **FR-104** | 소설 | 조회 | 검색 | 키워드 검색 | 제목, 작가명(필명), 태그로 소설 검색 가능. |
| **FR-105** | 소설 | 조회 | 상세 | 문장 리스트 | 소설 상세 진입 시 작성된 문장들을 순서대로 나열. 작성자/작성일시 표시. |
| **FR-106** | 소설 | 조회 | 상세 | 완료 보기 | 완결된 소설은 'Reader View' (페이지 넘김 효과 등) 제공. |
| **FR-201** | 집필 | 작성 | 검증 | 순서 제어 | 현재 `current_sequence`에 해당하는 순서만 작성 가능. |
| **FR-202** | 집필 | 작성 | 규칙 | 연속 작성 불가 | 직전 문장을 쓴 사용자는 바로 다음 문장을 쓸 수 없다 (`last_writer_id` 체크). |
| **FR-203** | 집필 | 작성 | 기능 | 문장 등록 | 100자 이내의 문장을 작성하여 등록. 등록 즉시 `current_sequence` 증가. |
| **FR-204** | 집필 | 상태 | 변경 | 소설 완결 | 설정된 `max_sequence` 도달 시 소설 상태가 `COMPLETED`로 자동 변경. |
| **FR-301** | 반응 | 투표 | 소설 | 좋아요/싫어요 | 소설 자체에 대한 추천/비추천. (IP/ID당 1회 제한). |
| **FR-302** | 반응 | 투표 | 문장 | 문장 평가 | 특정 문장에 대한 좋아요(BEST 문장 선정용). |
| **FR-303** | 반응 | 댓글 | 작성 | 감상평 | 소설 하단에 감상평(댓글) 작성 가능. (문장 중간 코멘트는 제외). |
